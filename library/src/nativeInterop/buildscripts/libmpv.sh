...
#    /home/toaster/Projects/Kotlin/spmp/spmp-server/library/src/nativeInterop/buildscripts/build/libmpv/output/lib
...
if [ ! -d libplacebo ]; then
    ...
    git clone https://code.videolan.org/videolan/libplacebo.git --depth=1 --recursive --single-branch -b v6.338.2
fi

if [ ! -d ffmpeg ]; then
    echo "Downloading ffmpeg..."

    git clone https://git.ffmpeg.org/ffmpeg.git --depth=1 --recursive --single-branch -b n7.0
fi
...
cd libass
LDFLAGS="-L$OUTPUT_DIR/lib" ./configure \
    --prefix=$OUTPUT_DIR \
    --disable-libunibreak \
...
export CC="$CC -I$OUTPUT_DIR/include"
